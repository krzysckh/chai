(define-library
  ...
  (begin
    ...
    (define (write-time-db d)
      (let ((fname (string-append d "/times.scm")))
        (when (not (file? fname))
          (let ((f (open-output-file fname)))
            (print-to f "'()")
            (close-port f)))

        (let* ((db (read-config fname))
               (imgs (get-images d))
               (data (fold (λ (a b)
                             (if (has? (map car a) b)
                                 a
                                 (append a `((,b . ,(date-str (time)))))))
                           db
                           imgs))
               (o (open-output-file fname)))
          (write-bytes o (string->list (str* data)))
          (close o)
          data)))
      ...
      (let* ((css (aq 'css cfg))
        ...
        (for-each
         (λ (s)
           ...
           (system-say `("convert" ,S "-thumbnail" "300x300"
